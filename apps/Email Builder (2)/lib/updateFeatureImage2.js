...
if (!featureImage2.value || featureImage2.value.length === 0) {
  throw new Error("No image has been uploaded to featureImage2.");
}
...
const imageInfo = featureImage2.value[0]; // Use the first (and likely only) uploaded image
...
(async () => {
  try {
    ...
    const blockRegex = /<!--\s*FEATURE 2\s*-->([\s\S]*?)<!--\s*\/FEATURE 2\s*-->/g;
 ...
 if (featureImgWidthState2.value >= 532 && featureSelect2.value != "None") {
  newHeight = Math.floor(newHeight / 2);
} else if (featureImgWidthState2.value >= 468 && featureSelect2.value == "None") {
  newHeight = Math.floor(newHeight / 2);
}
      ...
      // Updated comments to FEATURE 1 and /FEATURE 1
      return `<!-- FEATURE 2 -->${updatedInnerHtml}<!-- /FEATURE 2 -->`;
    };
