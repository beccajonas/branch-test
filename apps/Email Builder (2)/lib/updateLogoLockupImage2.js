// First, ensure we have a file uploaded to logoLockupImage2
if (!logoLockupImage2.value || logoLockupImage2.value.length === 0) {
  throw new Error("No image has been uploaded to logoLockupImage2.");
}
...
const imageInfo = logoLockupImage2.value[0]; // Use the first (and likely only) uploaded image
...
(async () => {
  ...
  : `<!-- / Bookline ${booklineSelect2.value} -->`;
    ...
    //  const booklineComment = `<!-- / Bookline ${booklineSelect2.value} -->`;
    ...
    const featureEndComment = featureSelect2.value === feature_image_left.value ?
      '<!-- /Master Email Feature Left -->' :
      '<!-- /Master Email Feature Right -->';
    ...
    return "Logo lockup image 2 updated successfully.";
  } catch (error) {
    throw new Error("Error updating logo lockup image 2: " + error.message);
  }
})();
