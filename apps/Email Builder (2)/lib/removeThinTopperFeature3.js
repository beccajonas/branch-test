...
const enclosingStartComment = '<!-- FEATURE 3 -->';
const enclosingEndComment = '<!-- /FEATURE 3 -->';
...
if (enclosingMatch) {
  ...
  const sheetRowNumber = parseInt(startRow3.value, 10);
  ...
  if (isNaN(sheetRowNumber) || sheetRowNumber < 1) {
    ...
    return "Invalid row number. Thin topper removed by default from Feature 3 block.";
  }
  ...
  if (targetRow) {
    ...
    if (topperValue && String(topperValue).trim() !== '') {
      updatedFullEnclosingBlockContent = updatedFullEnclosingBlockContent.replace(thinTopperRegex, '');
      console.log("Thin topper removed from Feature 3 block because topper content was found.");
    } else {
      console.log("Thin topper kept in Feature 3 block because topper content was empty or not found.");
    }
  } else {
    updatedFullEnclosingBlockContent = updatedFullEnclosingBlockContent.replace(thinTopperRegex, '');
    console.log("Thin topper removed from Feature 3 block because no corresponding sheet row was found.");
  }
  ...
  return "Thin topper check and removal completed for Feature 3 block.";

} else {
  throw new Error(`Enclosing comment block starting with '${enclosingStartComment}' not found in HTML content.`);
}

return "No relevant changes made to Feature 3 block."; // Fallback message if no match
