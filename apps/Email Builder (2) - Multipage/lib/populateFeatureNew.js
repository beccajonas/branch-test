...
if (getSheetsData.data && getSheetsData.data.length > 0) {
  ...
  const startingRow = startRow1.value - 1;
  ...
  if (featureSelect1.value === feature_image_right.value) {
  ...
  } else if (featureReskinSelect1.value != "None") {
    ...
    throw new Error("Invalid featureSelect1 value");
  }
  ...
  let booklineComment; // Renamed from const to let because it's assigned conditionally
  if (booklineSelect1.value === 0) { // New: Handle the specific "DeviceBookline" string value
    booklineComment = '<!-- /Hero -->';
  } else if (booklineSelect1.value === "DeviceBookline") { // Handle the specific "DeviceBookline" string value
    booklineComment = '<!--/ Device Bookline WAS/NOW-->'; // Use the exact updated comment
  } else if (deviceSwitch.value) { // Handle numerical device content card
    booklineComment = `<!-- /Device Content Card ${booklineSelect1.value} -->`;
  } else { // Handle numerical regular bookline
    booklineComment = `<!-- / Bookline ${booklineSelect1.value} -->`;
  }
  ...
  featureObj.forEach((obj) => {
    ...
    let booklineFound = false; // This seems to be used to stop iterating through sheets data
  ...
  const featureReskinLabel = featureReskinSelect1.selectedLabel;
  if (
  ...
  if (topperRemoved && !logoSwitch1.value) {
