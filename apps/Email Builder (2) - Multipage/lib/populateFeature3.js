...
if (getSheetsData.data && getSheetsData.data.length > 0) {
  ...
  const startingRow = startRow3.value - 1;
  ...
  } else if (featureSelect3.value === trustpilot_review.value) {
    featureObj = trustpilotObj.value;
    featureStartComment = '<!--TRUSTPILOT REVIEW-->';
    featureEndComment = '<!--/ TRUSTPILOT REVIEW-->'
  } else if (featureReskinSelect3.value != "None") { // Changed from featureReskinSelect1
    featureObj = ftReskinObj.value;
    featureStartComment = '<!-- FEATURE 3 -->'; // Comment updated for Feature 3
    featureEndComment = '<!-- /FEATURE 3 -->'; // Comment updated for Feature 3
  } else {
    throw new Error("Invalid featureSelect3 value"); // Changed from featureSelect2
  }
  ...
  let booklineComment;
  if (booklineSelect3.value === 0) {
    booklineComment = '<!-- /Hero -->';
  } else if (booklineSelect3.value === "DeviceBookline") {
    booklineComment = '<!--/ Device Bookline WAS/NOW-->';
  } else if (deviceSwitch.value) {
    booklineComment = `<!-- /Device Content Card ${booklineSelect3.value} -->`;
  } else {
    booklineComment = `<!-- / Bookline ${booklineSelect3.value} -->`;
  }
  ...
  // IMPORTANT: Changed featureReskinSelect1 to featureReskinSelect3
  const featureReskinLabel = featureReskinSelect3.selectedLabel;
  if (
  ...
  // IMPORTANT: Changed logoSwitch1 to logoSwitch3
  if (topperRemoved && !logoSwitch3.value) {
  ...
  // Replace FEATURE1_LINK with FEATURE3_LINK (Changed from FEATURE2_LINK)
  featureHtml = featureHtml.replace(/FEATURE1_LINK/g, 'FEATURE3_LINK');
