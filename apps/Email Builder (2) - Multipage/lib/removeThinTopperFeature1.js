...
if (enclosingMatch) {
  ...
  const sheetRowNumber = parseInt(startRow1.value, 10);
  ...
  if (targetRow) {
    ...
    if (topperValue && String(topperValue).trim() !== '') {
      updatedFullEnclosingBlockContent = updatedFullEnclosingBlockContent.replace(thinTopperRegex, '');
      console.log("Thin topper removed from Feature 1 block because topper content was found.");
    } else {
      console.log("Thin topper kept in Feature 1 block because topper content was empty or not found.");
    }
  } else {
    updatedFullEnclosingBlockContent = updatedFullEnclosingBlockContent.replace(thinTopperRegex, '');
    console.log("Thin topper removed from Feature 1 block because no corresponding sheet row was found.");
  }
  ...
  return "Thin topper check and removal completed for Feature 1 block.";

} else {
  throw new Error(`Enclosing comment block starting with '${enclosingStartComment}' not found in HTML content.`);
}

return "No relevant changes made to Feature 1 block."; // Fallback message if no match
