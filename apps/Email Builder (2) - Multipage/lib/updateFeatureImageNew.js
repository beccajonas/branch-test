...
if (!featureImage1.value || featureImage1.value.length === 0) {
  throw new Error("No image has been uploaded to featureImage1.");
}
...
const imageInfo = featureImage1.value[0]; // Use the first (and likely only) uploaded image
...
(async () => {
  try {
    ...
    const blockRegex = /<!--\s*FEATURE 1\s*-->([\s\S]*?)<!--\s*\/FEATURE 1\s*-->/g;
 ...
 if (featureImgWidthState1.value >= 532 && featureSelect1.value != "None") {
  newHeight = Math.floor(newHeight / 2);
} else if (featureImgWidthState1.value >= 468 && featureSelect1.value == "None") {
  newHeight = Math.floor(newHeight / 2);
}
      ...
      // Updated comments to FEATURE 1 and /FEATURE 1
      return `<!-- FEATURE 1 -->${updatedInnerHtml}<!-- /FEATURE 1 -->`;
    };
